...
import { Building2, Calendar, MessageSquare, Trash2, CheckCircle, XCircle, ChevronRight, ShieldCheck, Edit, Smartphone } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'properties' | 'bookings' | 'messages'>('properties');
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <aside className="w-full md:w-64 space-y-2">
            ...
            <nav className="space-y-1">
              {[{ key: 'properties', label: '房源管理', Icon: Building2 }, { key: 'bookings', label: '預約看房', Icon: Calendar }, { key: 'messages', label: '租客訊息', Icon: MessageSquare }].map(({ key, label, Icon }) => (
          ...
          <main className="flex-1">
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-900">{activeTab === 'properties' && '房源列表'}{activeTab === 'bookings' && '預約請求'}{activeTab === 'messages' && '對話紀錄'}</h2>
