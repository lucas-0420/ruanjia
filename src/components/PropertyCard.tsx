    ...
    <div className="group bg-white rounded-3xl overflow-hidden border border-[#E5D5C5] hover:shadow-xl hover:shadow-[#F5A623]/10 transition-all duration-500">
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <button
          ...
          className={cn(
            "absolute top-3 right-3 sm:top-4 sm:right-4 p-2 sm:p-2.5 rounded-full transition-all duration-300 shadow-md",
            isFavorite
              ? "bg-[#F5A623] text-[#3D2B1F]"
              : "bg-white/85 backdrop-blur-sm text-[#B8A090] hover:text-[#F5A623]"
          )}
        >
          <Heart className={cn("w-4 h-4 sm:w-4.5 sm:h-4.5", isFavorite && "fill-current")} />
        </button>
      ...
      <Link to={`/property/${property.id}`} className="p-4 sm:p-5 block">
        {/* 位置與樓層 */}
        <div className="flex items-center gap-1.5 text-[#9A7D6B] text-xs font-medium mb-2">
        ...
        {/* 標題 */}
        <h3 className="text-sm sm:text-base font-bold text-[#3D2B1F] mb-3 line-clamp-1 group-hover:text-[#F5A623] transition-colors">
          {property.title}
        </h3>
        ...
        <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[#F2E9DF]">
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] sm:text-[10px] text-[#B8A090] font-bold">格局</span>
            <div className="flex items-center gap-1 text-[#3D2B1F] font-bold text-xs sm:text-sm">
              <Bed className="w-3.5 h-3.5 text-[#F5A623]" />
              {property.features.bedrooms} 房
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] sm:text-[10px] text-[#B8A090] font-bold">衛浴</span>
            <div className="flex items-center gap-1 text-[#3D2B1F] font-bold text-xs sm:text-sm">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] sm:text-[10px] text-[#B8A090] font-bold">坪數</span>
            <div className="flex items-center gap-1 text-[#3D2B1F] font-bold text-xs sm:text-sm">
